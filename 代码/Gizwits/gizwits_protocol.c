...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagAll)
    {
        ...
        info->num++;
        dataPoints->valueAll = gizStandardDecompressionValue(All_BYTEOFFSET,All_BITOFFSET,All_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagVoice)
    {
        info->event[info->num] = EVENT_Voice;
        info->num++;
        dataPoints->valueVoice = gizStandardDecompressionValue(Voice_BYTEOFFSET,Voice_BITOFFSET,Voice_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    {
        GIZWITS_LOG("valueAll Changed\n");
        ret = 1;
    }
    if(last->valueVoice != cur->valueVoice)
    {
        GIZWITS_LOG("valueVoice Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(All_BYTEOFFSET,All_BITOFFSET,All_LEN,(uint8_t *)devStatusPtr,dataPoints->valueAll);
    gizStandardCompressValue(Voice_BYTEOFFSET,Voice_BITOFFSET,Voice_LEN,(uint8_t *)devStatusPtr,dataPoints->valueVoice);
    if(dataPoints->valueLED >= LED_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueLED Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
